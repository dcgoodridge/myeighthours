...
import myeighthours.viewcontroller.AboutController;
...
public class HyperlinkRedirectListener implements ChangeListener<Worker.State>, EventListener {
    ...
    public void handleEvent(Event event) {
        ...
        if (Desktop.isDesktopSupported()) {
            ...
            LOG.warn("OS does not support desktop operations like browsing. Cannot open link '{}'.", href);
        }
        event.preventDefault();
    }

    private void openLinkInSystemBrowser(String url) {
        LOG.debug("Opening link '{}' in default system browser.", url);
        try {
            URI uri = new URI(url);
            Desktop.getDesktop().browse(uri);
        } catch (Throwable e) {
            LOG.error("Error on opening link '{}' in system browser.", url);
        }
    }
